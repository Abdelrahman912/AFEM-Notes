...
md""" ## Newton-Raphson 1D

"""

# ╔═╡ fae32b1f-0169-4d18-9eff-b27bb9beaa88
load("./Resources/taylor.png")
...
load("./Resources/linear_taylor.jpg")
...
-\boldsymbol{f}(\boldsymbol{u}_k) = \boldsymbol{K}_t \cdot \Delta \boldsymbol{u} \; \; \Rightarrow \; \; \Delta \boldsymbol{u} = - \boldsymbol{K}_t^{-1} \cdot \boldsymbol{f}(\boldsymbol{u}_k) \\

\boldsymbol{K}_t = \frac{\partial f(\boldsymbol{u}_k)}{\partial u} \\
\boldsymbol{u}_{k+1} = \boldsymbol{u}_k + \Delta \boldsymbol{u}
...
begin
	function read_video_as_base64(file_path)
	    video_file = open(file_path, "r")
	    video_data = read(video_file)
	    close(video_file)
	    return base64encode(video_data)
	end
	
	function display_video(file_path)
	    video_base64 = read_video_as_base64(file_path)
	    video_html = """
	    <video  controls>
	...
	display_video("./Resources/NewtonRaphson.mp4")
end
...
# ╔═╡ c49314cc-bf70-4729-94a6-6f464dff10ee
